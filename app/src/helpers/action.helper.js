...
export const syncUpdateParams = (params) => {
    let user = getUser();

    params = Object.assign(
        {},
        { update_by: `${user.first_name} ${user.last_name}` },
        params
    );
...
export const syncCreateParams = (params) => {
    let user = getUser();
...
export const syncDeleteParams = (newParams) => {
    let user = getUser();
    ...
    if (search) {
        ...
        if (search.length > 0) {
            search.map((value) => {
                ...
                if (valueArr.length <= 1) return value;
...
export const convertIfNull = (value) => {
    if (!value)
        return (value = <span className="text-muted font-italic">Null</span>);
};

export const dateConvertIfNull = (value) => {
    if (value) return (value = moment(value).format("lll"));
